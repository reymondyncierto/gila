      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            className="w-full px-4 py-2.5 rounded-lg bg-white border border-slate-200 text-slate-900 text-sm placeholder-slate-400 focus:outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100 transition-all"
            placeholder="e.g., Personal Gmail"
            required
          />
        </div>

        <div className="space-y-4 bg-white rounded-xl p-5 border border-slate-200 shadow-sm">
          {currentFields.map((f) => (
            <div key={f.key} className="space-y-1.5">
              ...
              {f.key === "body" ? (
                <textarea
                  ...
                  className="w-full px-4 py-2.5 rounded-lg bg-slate-50 border border-slate-200 text-slate-900 text-sm placeholder-slate-400 focus:outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100 transition-all min-h-[120px] resize-y"
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    value={fields[f.key] || ""}
                    onChange={(e) => updateField(f.key, e.target.value)}
                    className="flex-1 px-4 py-2.5 rounded-lg bg-slate-50 border border-slate-200 text-slate-900 text-sm placeholder-slate-400 focus:outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100 transition-all"
                    placeholder={f.placeholder}
                    required={f.required}
                  />
                  {f.type === "password" && (
                    <button
                      ...
                      className="px-3 py-2.5 text-xs rounded-lg bg-sky-50 text-sky-500 font-medium hover:bg-sky-100 transition-colors shrink-0"
                    >
                      Generate
                    </button>
                  )}
        ...
        <div className="flex gap-3 justify-end">
          ...
          <button
            type="submit"
            disabled={loading}
            className="px-5 py-2 text-sm rounded-lg bg-sky-500 text-white font-medium hover:bg-sky-600 disabled:opacity-50 transition-colors shadow-sm"
          >
